...
class StaffButtons : public Staff{public:
    Staff workers;
    void notifyNurses(){
        if (ch == '8'){        workers.alertNurses();
        }    }    void notifySecurity(){        if (ch == '7'){        workers.alertSecurity();        }    }    void getPatientNames(){        if (ch == '6'){            cout << "--Getting patients Names" << endl;            for (it=myvector.begin(); it < myvector.end(); it++)            std::cout << '\t' << *it << endl;            std::cout << '\n';            getch();        }    }    void selectPatientFile(){        if (ch == '5'){        while(1){        getch();            if (ch == '+'){                myvector.begin()++;                cout << *it;            }            if (ch == '-') {                myvector.begin()--;                cout << *it;            }        }        }    }};class Patients : public Staff{
};int main(){
